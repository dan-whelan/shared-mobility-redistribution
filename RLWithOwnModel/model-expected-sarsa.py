    ...
    try:
        for episode in range(1, num_episodes + 1):
            ...
            while not done:
                ...
                best_action = np.argmax(Q[next_state_tuple])
                policy[best_action] += 1 - epsilon
                expected_value = np.dot(Q[next_state_tuple], policy)
                Q[state_tuple + (action,)] += alpha * (reward + gamma * expected_value - Q[state_tuple + (action,)])
